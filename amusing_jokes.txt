...
int main()
{
    ...
    if(guest_name.size() + host_name.size() == pile_of_letters.size())
    {
        for(i=0; i < pile_of_letters.size(); i++)
        {
            ...
            for(j=0; j < guest_name.size(); j++)
            {
                count1 = count(guest_name.begin(),guest_name.end(),pile_of_letters[i]);
                for(k=0; k < host_name.size(); k++)
                {
                    count2 = count(host_name.begin(),host_name.end(),pile_of_letters[i]);
                } 
           }
           if(count_val == count1 + count2)
           {
                flag = 0;
                continue;
           }
